...
if ( ! function_exists( 'storefront_site_title_or_logo' ) ) {
	...
	function storefront_site_title_or_logo( $echo = true ) {
		if ( function_exists( 'the_custom_logo' ) && has_custom_logo() ) {
			...
			$html = '<' . esc_attr( $tag ) . ' class="beta site-title text-red-700"><a href="' . esc_url( home_url( '/' ) ) . '" rel="home">' . esc_html( get_bloginfo( 'name' ) ) . '</a></' . esc_attr( $tag ) . '>';
...
if ( ! function_exists( 'storefront_primary_navigation' ) ) {
	...
	function storefront_primary_navigation() {
		?>
		<nav id="site-navigation" class="main-navigation flex flex-col items-center z-40" role="navigation" aria-label="<?php esc_attr_e( 'Primary Navigation', 'storefront' ); ?>">
		<button class="menu-toggle hidden" aria-controls="site-navigation" aria-expanded="false"><span><?php echo esc_html( apply_filters( 'storefront_menu_toggle_text', __( 'Menu', 'storefront' ) ) ); ?></span></button>
			<?php
			wp_nav_menu(
				array(
